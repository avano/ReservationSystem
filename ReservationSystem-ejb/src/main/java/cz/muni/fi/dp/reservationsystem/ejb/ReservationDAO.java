...
public class ReservationDAO implements ReservationDAOLocal {
    ...
    public Reservation getReservationById(long id) {
        Query q = em.createQuery("select reservation from Reservation reservation where id = ?1");
        q.setParameter(1, id);
        Reservation r = null;
        try {
    ...
    public List<Reservation> getReservationsByUser(User user) {
        Query q = em.createQuery("select reservation from Reservation reservation where user_id = ?1");
    ...
    public List<Reservation> getReservationsByComputer(long computer) {
        Query q = em.createQuery("select reservation from Reservation reservation where computer_id = ?1");
    ...
    public List<Reservation> getAllReservations() {
        Query q = em.createQuery("select reservation from Reservation reservation");
        return q.getResultList();
    }
    ...
    public void addReservation(User user, Computer computer, Date since, Date until) {
        java.sql.Date server_since = new java.sql.Date(since.getTime()); // kvoli openshift db
        java.sql.Date server_until = new java.sql.Date(until.getTime()); // kvoli openshift db
    ...
    public List<Reservation> checkInterval(long comp_id, Date since, Date until) {
        java.sql.Date sql_since = new java.sql.Date(since.getTime());
        java.sql.Date sql_until = new java.sql.Date(until.getTime());
        Query q = em.createQuery("select reservation from Reservation reservation where computer_id = ?1 and ((until between ?2 and ?3) or (since between ?2 and ?3) or (since >= ?2 and until <= ?3) or (since <= ?2 and until >= ?3))");
    ...
    public void deleteReservation(Reservation r) {
        Reservation res = em.find(Reservation.class, r.getId());
        em.remove(res);
        em.flush();
    }
    ...
    public boolean update(Reservation current) {
        List<Reservation> resInInterval = checkInterval(current.getComputer().getId(), current.getSince(), current.getUntil());
    ...
    public List<Reservation> getCurrentReservationsByUser(User user) {
        ...
        Query q = em.createQuery("select reservation from Reservation reservation where user_id = ?1 and ((since > ?2) or (since <= ?2 and until >= ?2))");
        q.setParameter(1, user.getId());
        q.setParameter(2, d);
        return q.getResultList();
    }
    ...
    public List getCurrentReservationsByComputer(long comp_id) {
        java.sql.Date d = new java.sql.Date(new Date().getTime());
        Query q = em.createQuery("select reservation from Reservation reservation where computer_id = ?1 and ((since > ?2) or (since <= ?2 and until >= ?2))");
        q.setParameter(1, comp_id);
        q.setParameter(2, d);
        return q.getResultList();
    }
    ...
    public void deleteAllReservationsByUser(User user) {
        Query q = em.createQuery("select reservation from Reservation reservation where user_id = ?1");
        q.setParameter(1, user.getId());
        List<Reservation> ret = q.getResultList();
    ...
    public List<Reservation> getLastMonthReservationsByComputer(long computerId) {
        ...
        Query q = em.createQuery("select reservation from Reservation reservation where computer_id = ?1 and ((since >= ?2) or (until <= ?3))");
    ...
    public List<Reservation> getLastMonthReservations() {
        ...
        cal.set(Calendar.MILLISECOND, 999);
        java.sql.Date today = new java.sql.Date(cal.getTimeInMillis());
        Query q = em.createQuery("select reservation from Reservation reservation where ((until > ?1 and until <= ?2) or (since <= ?1 and until >= ?2) or ((since between ?1 and ?2) and until >= ?2))");
        q.setParameter(1, d);
    ...
    public List getCurrentReservations() {
        ...
        Query q = em.createQuery("select reservation from Reservation reservation where ((since > ?1) or (since <= ?1 and until > ?1)) order by id asc");
        q.setParameter(1, d);
        return q.getResultList();
    }
    ...
    public Reservation getReservationByToken(String token) {       
        Query q = em.createQuery("select reservation from Reservation reservation where token = ?1");
